      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <aside className="md:col-span-1 bg-white p-6 rounded-xl shadow-sm hidden md:block">
          ...
          <ul className="space-y-3 text-sm">
            ...
            {CATEGORIES.map((c) => (
              <li key={c.id}>
                <button onClick={() => setActiveCategory(c.title)} className={`w-full text-left ${activeCategory === c.title ? "font-semibold text-purple-600" : ""}`}>{c.title}</button>
              </li>
            ))}
          </ul>
        ...
        <section className="md:col-span-3">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {visible.length === 0 && <div className="col-span-full text-gray-500">No products found for the selected category or search.</div>}
